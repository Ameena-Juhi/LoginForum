...
public class StudentController {
	...
	public void processStudentForm(String name, String score, HttpServletResponse resp) throws IOException {
		// message = String.format("I am currently processing student with %s %s", name,
		// score);
		int scoreAsInt = Integer.valueOf(score);
		var student = new Student1(name, scoreAsInt);
	...
	public void deleteStudent(int id, HttpServletResponse resp) throws IOException {
		Optional<Student1> toEdit = students.stream().filter(student -> student.getId() == id).findFirst();
		if (!toEdit.isEmpty()) {
